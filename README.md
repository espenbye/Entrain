...
On iOS and watchOS the snapshot lives in the `group.no.espenbye.entrain` App Group instead, so device builds there need a development team; simulator builds do not. App Review rejects the path exception, so a Mac App Store archive signs with the App Group entitlements in `AppStore/` instead, and the app then uses the group container at runtime. The iCloud key-value store needs a certificate the same way, so those entitlements are also where the Mac gets settings sync; a development build of the Mac app keeps its settings to itself, while the iOS and watch targets carry the entitlement in every build:
