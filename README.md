...
A Mac, iPhone, iPad and Apple Watch app that plays a generated soundscape with rhythmic amplitude modulation, tuned to nudge your brain toward a target state. No audio files but the alarm's ring, no network. Everything else is synthesized in real time. On the Mac it lives in the menu bar.
...
**Sleep target.** Everything above is descriptive: the signature measures the fortnight and the day follows it, so an app used by someone going to bed at two quietly organises itself around two. That is right for a sound that has to fit the life it is played in and useless to someone trying to change that life, so Settings has one prescriptive switch — a wake time and a length, from which a bedtime follows. It is off by default, because an app that asks for a target on first launch is an app with an opinion about how you should live, and the lengths offered stop at six and ten hours, outside which Entrain has no business helping anyone aim.

On, the target replaces the sun and the signature as the two edges of the night: Wind Down opens three hours before the target bedtime, Wake covers the two hours before the target wake, and the suggestion says it is aiming rather than following. It does not ask for the whole change at once. A body clock will not jump — the phase-response literature puts the ceiling on an unaided advance at around half an hour a day (Duffy & Wright 2005; Burgess et al. 2003) — so tonight's edges are the measured habit moved twenty minutes toward the target and no further, and Your Day shows the target, tonight's step and the habit as three rows, because a target you cannot see yourself missing is a wish. There is no stored progress and no count of nights, and there needs to be none: the step is taken from the fourteen-night average, which creeps as the nights are kept, so keeping it is what advances the walk and stopping stalls it, which is the correct behaviour rather than a counter ticking down toward a bedtime nobody was keeping. A signature too scattered to be a habit is not walked from at all — that sleeper has no habit to ease out of and is exactly who a target is for — so the target applies from the first night instead.
...
Wake has an alarm on iPhone and iPad, built on AlarmKit, in the Alarm tab. Pick a time, optionally some weekdays, and switch it on; it rings through silent mode and Focus like a Clock alarm. With no days it rings once at the next occurrence and shows a Live Activity counting down until then, with a Cancel button. With days it repeats weekly, and the system re-arms it after each ring. The alarm's Start Wake button opens Entrain and plays the ramp, for 30 minutes when the timer is endless; Dismiss just silences it. It needs the alarm permission the first time.

**Scan to Stop** is the alarm for a heavy sleeper. The system alert always has a Stop button, and a heavy sleeper presses it without waking, so the alarm is not one alarm: with Scan to Stop on, the first ring is followed by twenty-nine more two minutes apart, each its own AlarmKit alarm, so Stop only silences the one ringing and the next is already on its way. The only thing that cancels the rest is scanning the code registered in the tab — any barcode or QR code that lives where you want to end up, like the toothpaste — and doing that starts Wake. While a volley is going the whole app is the scanner, with no way past it, and it sounds the ring itself, in the background too, until the code is scanned. The ring is a rendered file, the one audio file in the app, because AlarmKit plays files: full-scale beeps pitched where hearing is most sensitive, as loud as the ringer volume allows. The first ring is the ordinary repeating alarm, so a morning slept through does not cost the next one; the followers are one-shot alarms re-armed for the next morning after every scan and whenever the app comes forward. The volley stops on its own after an hour, long enough that nobody sleeps through it and short enough that a phone left at home is not ringing at lunch. The system caps how many alarms an app may hold and does not say where; a volley cut short at the cap keeps the rings it got, and the tab says how many. There is deliberately no other task and no fallback. Steps can be shaken out of a phone lying down, sums can be done half asleep, and a fallback is a way past.
...
Tools/
  ...
  alarm.swift  Renders the alarm's ring, Resources/Alarm.caf
Scripts/
